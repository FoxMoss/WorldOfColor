...
func _process(delta):
	...
	visable_health = lerp(visable_health, health, 0.1)
	($Sprite.material as ShaderMaterial).set_shader_param("bubblelevel", 1 - visable_health)
	
	
	if(visable_health <= 0.01):
		self.queue_free()
